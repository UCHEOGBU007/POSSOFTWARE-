...
export default function StaffPage() {
  ...
  const handleSave = async () => {
    if (!form.name.trim() || !form.email.trim() || !form.outletId) {
      showError("Name, email, and branch assignment are required.");
      return;
    }

    if (!editingStaff) {
      if (!form.pin || form.pin.length < 12) {
        showError("Staff password must be at least 12 characters.");
        return;
      }
      ...
      if (editingStaff) {
        ...
        if (updatedStaff) {
          ...
          if (!syncResult.ok) {
            throw new Error(
              (syncResult.error as Error)?.message ||
                "Failed to sync updated staff details.",
            );
          }
        }
        ...
        success(`Staff member "${form.name}" created successfully.`);
      }
          ...
          {!editingStaff ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Input
                ...
                type={showPin ? "text" : "password"}
                placeholder="Unique staff password"
                value={form.pin}
            ...
            <p className="text-xs text-pos-muted italic bg-pos-bg p-3 rounded-lg border border-pos-border/40">
              Note: Staff passwords cannot be changed from this screen.
              Passwords are managed directly via Supabase Auth or password reset
              workflows.
            </p>
          )}
        </div>
